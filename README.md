...
The repository contains a public landing page, email/password sign-in, and one protected project route at `/app`. That route reads the seeded Supabase project, accepts source updates, renders persisted impact-review data, presents recovery actions, and reads operation history. The current integration limits are listed under [Known limitations](#known-limitations); this README does not claim a verified live model or production browser run.
...
Step 7 is currently blocked for a fresh analysis: analysis finalization persists the proposal as `draft`, while the apply contract accepts `ready` or `partially_approved`, and no promotion route exists. The UI shows this limitation and disables approval rather than bypassing it.
...
| Validation | Zod |
| Tests | Vitest, Testing Library, Playwright configuration, SQL assertion suites |
...
- surfaced the `draft` versus `ready` proposal-state mismatch instead of hiding it with a client-side workaround.
...
| `npm run test:e2e` | Run Playwright tests when an authenticated browser environment is available. |
...
| Fresh analyses persist proposals as `draft`; apply accepts `ready` or `partially_approved`. | The UI truthfully disables approval, and there is no promotion route on this branch. |
| `/app` is the only protected project page. | Items, dependencies, source review, proposals, history, and controls share one project screen; dedicated project-view routes are not part of this branch. |
| Demo reset has a server API but no dedicated reset control in the current protected UI. | The reviewed route and backend contract are documented in `docs/architecture.md`; authenticated browser verification remains pending. |
...
| Playwright end-to-end coverage is not complete for the authenticated workflow. | Component, unit, build, linked SQL/RPC, and labeled preview evidence do not replace production browser QA. |
...
- Resolve the proposal readiness transition and complete the authenticated evidence-to-undo browser journey.
- Add dedicated project item, decision, risk, and dependency views without duplicating canonical state.
...
- Repository: **`<PUBLIC_REPOSITORY_URL>`** — confirm public access before submission.
