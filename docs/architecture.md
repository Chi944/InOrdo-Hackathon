...
Residual operational risks are tracked rather than hidden: Vercel's outer body limit remains defense in depth while the application streams and cancels at its tighter caps; stale-claim reconciliation is demand-driven and therefore requires an exact POST replay after the displayed lease delay; and per-actor limits do not replace workspace-wide abuse monitoring. A project revision change makes the old logical claim irrelevant to new intake, but it does not rewrite that historical row. Earlier linked migrations and RLS/grant behavior are verified, but `20260721100000_add_analysis_access_policy.sql` has not been applied to the linked/hosted project and `supabase/tests/verify_analysis_access_policy.sql` has not been run there. Linked parity, the policy verifier, health, and viewer-denial checks remain explicit release-plan gates; local or mocked evidence must not be presented as linked verification.
