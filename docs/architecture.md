...
`(workspace_id, project_id, project_revision, normalized_content_sha256)` is the unique analysis key. The request-scoped client first verifies a non-anonymous contributor and loads one bounded project context. It then resolves one of three server-only modes: `recording`, `auto`, or `disabled`. An absent or invalid mode resolves to `disabled`. Recording mode can select only `gpt-5.6-luna` through `openai_recording`; auto mode can select only `openai/gpt-oss-20b` through `gateway_fallback`; disabled mode selects neither. Recording never falls back, and auto ignores `OPENAI_API_KEY`.

Only after those application checks does the lazy privileged persistence capability call `public.begin_project_analysis_with_policy`. This `SECURITY DEFINER`, empty-`search_path` wrapper is executable only by `service_role`, validates that database role, establishes the already verified actor's transaction-local claims, and invokes `private.begin_project_analysis_with_policy_internal` as the migration owner. The private function independently rechecks contributor membership, project scope, hashes, bounds, and current revision. No application role can execute the private function. Migration `20260721100000_add_analysis_access_policy.sql` also revokes `service_role` execution from the legacy `public.begin_project_analysis` and `private.begin_project_analysis_internal` bypasses; only the policy-aware public wrapper remains callable by the server capability.

Inside the same transaction and after the fixed source-key then actor-key advisory locks, the database resolves the existing duplicate first and the five-new-claims-per-actor/project rolling ten-minute rate limit second. It then resolves the mode and capability. Recording requires one unexpired `available` grant bound to the exact `(actor_id, project_id, normalized_content_sha256)` tuple. Grant lifetime is at most 15 minutes; issuance, revocation, and verification are private owner-only migration-owner functions with no application-role execution grant. A grant moves only `available -> claimed` or `available -> revoked`; its identity and terminal state are immutable. The exact grant is locked, but is not changed until immutable evidence and the request have been inserted. Claiming then links it to that exact request in the same transaction and requires exactly one updated row, so a hash, model, provenance, or claim invariant mismatch rolls back the evidence, request, and grant transition together.

Unavailable or disabled new requests return before creating evidence, a request, a provenance link, or a grant transition. A duplicate always returns before reading or consuming a grant. A duplicate in a configuration-ready allowlisted mode may preserve one distinct immutable source capture and provenance link, but it cannot create a second provider claim or model call; disabled, recording-unavailable, fallback-unavailable, and invalid-configuration duplicates create no new capture or link. The persisted request model remains the source of truth for the duplicate route.

Each newly claimed request receives one immutable, nonrenewable three-minute database lease—twice the route's 90-second maximum. An exact POST replay while the lease is active returns its bounded remaining delay. The first exact replay after expiry locks the row and atomically changes the existing request to failed with safe `persistence`/`analysis_cancelled` metadata; it never creates a new attempt, renews the lease, deletes evidence, or calls the model.

Evidence/claim creation is deliberately committed before model work so the original source and attempt remain auditable even when the provider refuses, times out, exhausts the capped fallback allowance, or returns invalid output. After the policy RPC returns a newly claimed provider route, and only then, the runtime constructs exactly that route's server-only adapter. A post-claim provider failure truthfully leaves immutable source evidence plus a failed analysis claim; it creates no proposal or project-item mutation. A failure transition records only an allowlisted stage/code and optional bounded provider request ID, never raw provider output or source text.

After both model calls and all application checks succeed, `complete_project_analysis` passes the same verified actor into a private implementation, locks the claim and graph tables against concurrent record writes, reauthorizes claim ownership/membership, recomputes the project revision, rechecks the changed item/version/current value, independently recomputes deterministic impact paths, and validates the entire derived payload. It then creates the pending change event, impact run/items, proposal/actions, and succeeds the claim in one transaction. The final state-transition guard rejects success at or after lease expiry; PostgreSQL rolls back every derived insert in that statement, so a late worker cannot beat reconciliation or leave partial records. A narrow completion trigger promotes only the exactly linked, current-generation proposal from `draft` to `ready` after confirming the change still needs confirmation, the impact run completed, and every one-to-eight action is pending and unattributed. Anomalous historical drafts stay quarantined. Readiness means eligible for human review, not approved: no action, item, or operation row changes during promotion. Any invalid element aborts the entire finalization; there are no partially written derived records. The completion and failure public wrappers remain `SECURITY INVOKER`, require a service-role JWT, and grant execution only to `service_role`; their validation implementations are private `SECURITY DEFINER` functions with empty `search_path`, fully qualified relations, and exact grants. The policy-aware begin wrapper's distinct `SECURITY DEFINER` boundary is documented above. Authenticated and anonymous browser roles cannot execute the persistence RPCs or directly update change/action review state.
...
Residual operational risks are tracked rather than hidden: Vercel's outer body limit remains defense in depth while the application streams and cancels at its tighter caps; stale-claim reconciliation is demand-driven and therefore requires an exact POST replay after the displayed lease delay; and per-actor limits do not replace workspace-wide abuse monitoring. A project revision change makes the old logical claim irrelevant to new intake, but it does not rewrite that historical row. Migration `20260721100000_add_analysis_access_policy.sql` is applied to the identity-confirmed linked project; exact parity, its rollback-wrapped verifier, linked lint, Production health, and viewer-denial checks are recorded in [`release-evidence.md`](release-evidence.md). Local or mocked evidence remains distinct and must not be presented as linked or Production verification.
...
The Supabase, authentication, repository, project-record, and deterministic-impact paths were implemented through Prompt 5. Prompt 7 adds the server-only evidence/analysis pipeline and pending proposal persistence. Prompt 9 adds the approval/application, ordered operation history, compensating undo, and history-preserving demo-reset backend contracts. The protected browser workflow is integrated, and linked migrations, schema/security checks, rollback-wrapped RPC/audit verification, the bounded Production analysis-to-undo journey, judge-viewer denial, and responsive Production browser matrix are complete. Exact release evidence and residual automated-versus-live limitations are recorded in [`release-evidence.md`](release-evidence.md).
