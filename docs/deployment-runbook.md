...
Deston must supply the OpenAI key, provision the Auth demo account, confirm current Vercel Hobby eligibility, and record the real Vercel/Supabase URLs. The team must complete the authenticated production smoke, funded synthetic analysis, final responsive/accessibility pass, public-video/Devpost links, team/deadline details, and primary `/feedback` Session ID. Placeholders are not release evidence and must never be filled with invented values.
