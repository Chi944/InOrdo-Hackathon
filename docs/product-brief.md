...
The integrated P0 contains the workspace database, RLS/authentication boundary, native item/dependency views, deterministic traversal, evidence/analysis workflow, eligible-ready but still inert proposals, selective approval, audit history, compensating undo, and named-demo reset UI. CI-safe browser coverage uses an explicitly labeled synthetic fixture and mocked external seams; it does not prove a live OpenAI, Supabase, RLS, or authenticated deployment path. That production smoke remains an operator release gate and must not be presented as complete until its non-secret evidence is recorded.
