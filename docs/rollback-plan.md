...
### Approval-integrity and create-receipt correction

`20260719130000_harden_approval_action_integrity` is also forward-only. If its proposal-set guard is suspect, contain analysis finalization and apply before accepting another proposal. Do not remove the trigger, reopen a proposal superseded by its migration backfill, edit immutable proposal actions, or relax the project-item date constraint. A corrective migration must preserve per-proposal serialization, reject duplicate target-field updates, and prove every selectively approvable non-null start/due pair is compatible in both insertion orders and under a concurrent race.
...
`20260719140000_guard_project_record_mutations` is the forward-only expand phase: it adds the four native mutation RPCs and private receipt ledger while temporarily retaining the legacy contributor policies/grants so the pre-RPC deployment remains schema-compatible. That artifact is not generation-safe: its direct item/dependency writes do not check `workflow_generation`. If one RPC or its ledger is suspect before contract, contain the new create/edit/dependency server actions and block the reset route at the deployment layer before rolling back to the inspected pre-RPC artifact; if reset containment cannot be proved for the whole rollback window, block all native mutation surfaces instead. Do not delete or rewrite a successful receipt, decrement `workflow_generation`, revive a stale action, or remove deferred reconciliation as a shortcut.

Direct authenticated table DML was removed by separately reviewed and approved contract migration `20260720190000` after the exact RPC-capable production artifact passed create, update, dependency-create, dependency-remove, replay, and reset smoke. A pre-RPC Vercel artifact is no longer a valid rollback target. Roll back only to an inspected RPC-capable artifact. If none is safe, contain the native mutation routes and ship a reviewed forward migration that restores only the minimum compatibility grant/policy needed for recovery; never edit or delete an applied migration or silently regrant broad browser writes.

Record only safe project, actor, mutation type, generation, item/dependency ID, version, idempotency-key fingerprint, state, and timestamp coordinates. Reconcile an ambiguous request by replaying the exact same request through the authorized RPC; never guess whether it committed. A corrective migration must retain project-row serialization, exact-success replay before generation validation, mismatch conflicts, no key consumption for rejected stale requests, append-only receipts, and the pending/approved-only staleness rule. Re-run every baseline and focused SQL verifier plus a real reset/mutation race before reopening writes. The expand and contract stages each require their own linked inventory, dry-run, exact typed approval, parity proof, and post-stage smoke; never apply both as one pending migration batch.
...
3. Before listing, rolling back, or aliasing an old deployment, complete credential-first containment: revoke every OpenAI recording key and Gateway key that could authorize the deployment, remove `OPENAI_API_KEY` and `AI_GATEWAY_API_KEY` from every Vercel scope where present, and privately prove provider-side invalidation plus name-only Vercel absence without displaying a value. If either credential can still work or removal cannot be proved, keep the current route contained and do not change the production alias.
4. List and inspect the known-good deployment, then repoint production with `npx --yes vercel@56.3.2 rollback <LAST_KNOWN_GOOD_DEPLOYMENT_URL_OR_ID>`. Verify the served deployment metadata and rerun `/api/health`, `/`, `/login`, signed-out `/app`, Auth, and the affected path. Preserve the reviewed Preview-only protection setting and do not delete the failed deployment to hide evidence.
5. A Vercel rollback changes only the served application artifact. It does not revert a database migration, undo an operation, or erase an analysis claim. The required credential invalidation/removal must already be complete before this step. Keep migrations forward-only and use the matching Prompt 7/9/10 recovery section for persisted state.

If bad non-provider environment configuration caused the incident, correct only the affected name in Vercel's secret store, never on the command line or in Git. Redeploy before testing because environment updates do not change an existing artifact. Provider credentials are governed by the mandatory credential-first rule above, not a conditional exposure test. Keep privileged routes contained and never print or record either an old or replacement value.
