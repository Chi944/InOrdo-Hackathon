...
- [x] **The RPC native-write path shares the reset fence; expand compatibility is explicitly weaker.** The application uses four allowlisted `SECURITY DEFINER` RPCs that reauthorize the current contributor, lock the project, check workflow generation and item version, and append one private idempotency receipt in the same transaction. Exact success replay precedes mutable owner and generation checks; stale first attempts consume no key. Migration `20260719140000` is deliberately expand-only and temporarily retains the legacy contributor DML path, which does not carry the generation predicate. A pre-RPC rollback is therefore only schema-compatible and requires deployment-layer reset containment (or containment of all native mutations) until the RPC-capable artifact returns. A separate reviewed contract migration must remove every legacy policy plus table- and column-level write grant after all four mutation smokes pass.
...
- [ ] Complete the native-mutation contract phase only after the exact deployed RPC artifact passes create, update, dependency-create, dependency-remove, and replay verification. Until then, direct authenticated project-record DML remains a knowingly temporary compatibility surface protected by the existing RLS contributor policies; after contract, pre-RPC artifacts are forbidden rollback targets.
