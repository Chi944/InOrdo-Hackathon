...
- [x] **Provider failures and refusals are bounded and user-safe in application code.** Each logical call has a 30-second timeout and SDK/request retries are disabled for the current 90-second route budget. Refusal, incomplete output, malformed output, timeout, transient failure, stale state, and persistence errors map to bounded public codes rather than raw provider or database detail.
...
- [ ] **Deployment evidence remains separate.** These checks prove the branch implementation, not that the production alias serves it. Merge, exact-SHA deployment inspection, readiness, and authenticated live smoke remain release gates in `docs/qa-checklist.md`.
