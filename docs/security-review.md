...
- [x] **Provider failures and refusals are bounded and user-safe in application code.** Each logical call has a 30-second timeout, `maxRetries: 0`, and no application retry loop. Refusal, incomplete output, malformed output, timeout, transient failure, stale state, and persistence errors map to bounded public codes rather than raw provider or database detail.
...
- [ ] Verify browser login, submission, duplicate feedback, pending review display, refusal/failure copy, and confirmation behavior against the deployed production services. The UI contract and guarded browser journey are implemented, but they are not credentialed live-service evidence.
- [x] The analysis route enforces its 24,000-byte application limit while streaming, cancels as soon as actual bytes exceed the cap even when `Content-Length` is absent or dishonest, and never buffers the complete oversized body. [Vercel's documented 4.5 MB Function request limit](https://vercel.com/docs/functions/limitations) remains an outer defense-in-depth boundary.
...
- [x] **The RPC native-write path shares the reset fence and the legacy path is contracted.** The application uses four allowlisted `SECURITY DEFINER` RPCs that reauthorize the current contributor, lock the project, check workflow generation and item version, and append one private idempotency receipt in the same transaction. Exact success replay precedes mutable owner and generation checks; stale first attempts consume no key. Migration `20260719140000` supplied the reviewed expand compatibility phase. After its four-mutation/replay smoke, separately approved contract migration `20260720190000` removed every legacy policy plus table- and column-level write grant. Hosted verification proves direct DML is denied while all four guarded RPCs and member reads remain available.
- [x] **Superseded proposal intent closes without fabricated review.** An initially deferred trigger re-reads the parent's final state and stales only pending or approved children. System-staled pending actions remain unattributed, approved actions retain real review attribution, terminal action history is preserved, and apply's temporary self-supersession does not stale its completed transaction.
...
- [x] Operation routes enforce their 32,000-byte application limit while streaming and cancel an oversized body without trusting `Content-Length`; the deployment limit remains defense in depth rather than the application memory bound.
...
- [x] The native-mutation contract phase completed after the exact RPC artifact passed create, update, dependency-create, dependency-remove, replay, and reset verification. Contract migration `20260720190000` is applied; direct authenticated project-record DML is denied, and pre-RPC artifacts are forbidden rollback targets.
...
- [x] **Operation requests are consistently bounded.** The UI/model/apply action count is one shared maximum of eight, a shared streaming reader returns safe `413` responses and cancels as soon as encoded bodies exceed 32,000 bytes, and database validation remains defense in depth.
...
- [ ] **Live deployment verification is partial.** Public Production routing, readiness, exact-SHA identity, skip-link focus, authenticated native mutations/reset, and hosted RLS/RPC contracts are verified. CI-safe Playwright still cannot prove the remaining fresh login/session/logout matrix, one funded provider response, analysis-to-undo browser path, or deployment monitoring. Follow `docs/qa-checklist.md` before presenting those as verified.

### Residual operational risks

- Vercel's documented 4.5 MB Function request limit remains defense in depth. The application no longer relies on it for the tighter analysis/operation caps: both handlers stream and count actual bytes, cancel over-limit bodies, and treat absent or dishonest length headers identically.
...
- [x] **Actual body bytes are bounded before orchestration.** Analysis and operation handlers share a streaming reader that counts cumulative encoded bytes, stops and cancels above 24,000/32,000 bytes, handles missing or dishonest `Content-Length`, rejects invalid UTF-8 safely, and never accumulates a complete oversized request.
- [x] **Readiness fails closed on unsafe configuration shapes.** Remote Supabase URLs require HTTPS; plaintext is limited to exact loopback development hosts; embedded URL credentials, padding, blank keys/secrets/slugs, and unsafe schemes are rejected. The health endpoint and runtime clients use the same parsers and expose only allowlisted variable names.
- [x] **Source rollback is target- and parent-aware.** The executable planner verifies the resolved target is an ancestor of synchronized `main`, rejects roots, octopus merges, disconnected commits, and invalid mainlines, and returns the exact parent used for both migration inventory and `git revert -m`. Explicit ordinary/merge commands avoid the empty-array `set -u` failure in macOS Bash 3.2.
