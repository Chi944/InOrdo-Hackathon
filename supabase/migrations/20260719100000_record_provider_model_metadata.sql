-- Persist the model name returned by each provider response separately from
-- the configured model alias claimed by the analysis request. This validator
-- remains an exact allowlist: arbitrary provider metadata is never accepted.
-- Historical rows are not backfilled because the provider-returned name
-- cannot be reconstructed safely from the configured alias.
...
begin
  if not private.jsonb_object_matches(
    ...
    or pg_catalog.jsonb_typeof(candidate -> 'model_name') <> 'string'
    or pg_catalog.char_length(candidate ->> 'model_name') not between 1 and 120
    or candidate ->> 'model_name' !~ '^[A-Za-z0-9._:/-]+$' then
    return false;
  end if;
